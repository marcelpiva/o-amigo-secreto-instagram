...
interface Post26HalloweenProps {
  slideIndex?: number;
  preview?: boolean;
}

// Fotos de Halloween - Unsplash royalty free
const slides = [
  {
    image: 'https://images.unsplash.com/photo-1509557965875-b88c97052f0e?w=1080&h=1080&fit=crop',
    title: 'AMIGO',
    highlight: 'FANTASMA',
    subtitle: 'Halloween com mistério',
    color: '#FF6600',
    emojis: [
      { emoji: '👻', top: 8, left: 78 },
  ...
  {
    image: 'https://images.unsplash.com/photo-1508361001413-7a9dca21d08a?w=1080&h=1080&fit=crop',
    title: 'TROCA DE',
    highlight: 'DOCES!',
    subtitle: 'Gostosuras ou travessuras',
    color: '#9C27B0',
    emojis: [
      { emoji: '🍬', top: 6, left: 80 },
  ...
  {
    image: 'https://images.unsplash.com/photo-1604141778873-d5de6c5c0f1d?w=1080&h=1080&fit=crop',
    title: 'ORGANIZE',
    highlight: 'A FESTA!',
    subtitle: 'Sorteio assustadoramente fácil',
    color: '#D4623A',
    cta: true,
    emojis: [
      { emoji: '🎉', top: 5, left: 78 },
      { emoji: '🕷️', top: 16, left: 88 },
      { emoji: '💀', top: 25, left: 82 },
    ],
  },
];

export function Post26Halloween({ slideIndex = 0, preview = true }: Post26HalloweenProps) {
  ...
  return (
    <PostFrame size="square" preview={preview} showLogo={slide.cta}>
      ...
      <div className="relative z-10 flex flex-col items-start justify-end h-full" style={{ padding: preview ? '20px' : '56px' }}>
        ...
        <div className="font-black leading-none mt-2 tracking-tight" style={{ fontSize: preview ? '34px' : '86px', color: slide.color, textShadow: `0 0 60px ${slide.color}90, 0 0 30px ${slide.color}70, 0 4px 15px rgba(0,0,0,0.8)` }}>{slide.highlight}</div>
        ...
        {slide.cta && (
          <div className="mt-5 flex flex-col gap-2">
            ...
            <div className="font-bold animate-pulse" style={{ fontSize: preview ? '12px' : '28px', padding: preview ? '10px 20px' : '16px 36px', background: `linear-gradient(135deg, ${slide.color} 0%, #9C27B0 100%)`, color: '#FFFFFF', boxShadow: `0 0 40px ${slide.color}70` }}>Boo! 👻</div>
          </div>
        )}
      </div>
